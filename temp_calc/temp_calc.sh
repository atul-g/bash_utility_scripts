...
while getopts 'hc:t:' flag; do
		...
		h)
			...
			echo -e  "\tUSAGE:${BOLD} ./temp_calc.sh [-c <value> ] [-t <value>]"
			...
			echo -e "${BOLD}\tNOTE: THIS SCRIPT DEPENDS ON stress PACKAGE${NORMAL}.\n\tInstall it using \"sudo apt install stress\"\n"
			exit 0;;
...
temp_measure() {
	...
	echo -e "\tThe maximum temperature during the test was: ${RED_COLOR}$((max_temp/1000)) celsius\n"
	tput sgr0
	exit 0
}
